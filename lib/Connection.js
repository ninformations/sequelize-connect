...
class Connection {
  ...
  constructor(database, username, password, options={}, discover=["/model"], matcher=null, logger=false, sync=true) {
  ...
  _connect(){
    ...
    return discoverable.discover()
      ...
      .then(() => {
        if(this.sync_enabled) this._log("info", "Finished synchronizing " + this.database);
        // Expose objects
